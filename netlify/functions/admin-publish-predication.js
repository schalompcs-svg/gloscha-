...
exports.handler=async event=>{
 ...
 try{
  ...
  const publication={id,titre,contenu,texte:contenu,date:d.date||new Date().toISOString(),theme:String(d.theme||'').trim(),orateur:String(d.orateur||'').trim(),introduction:String(d.introduction||'').trim(),verset:String(d.verset||'').trim(),photos}; await store.setJSON(id,publication);
  return json(200,{ok:true,publication,message:'Prédication ajoutée sans remplacer les précédentes.'},event);
